...
- **Presence / geofence alerts** (`watch`) — watch an identifier (BSSID/SSID,
  ...
  task for continuous edge-triggered alerting via webhook/email/NDJSON, plus a
  dashboard **Presence** panel (live status + add/remove, and click a scanned
  AP/device to watch it). New endpoints: `/api/presence`, `POST /api/watch/*`.
